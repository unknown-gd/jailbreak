...
do
	...
	local function looc( self, text, isTeamChat )
		return sendChatText( self:GetNearPlayers( VoiceChatMinDistance:GetInt(), true, false, false ), self, CHAT_LOOC, text, self:Nick(), self:Alive(), isTeamChat and self:Team() or false )
	end
	Jailbreak.LOOC = looc
...
do
	...
	local function customEmotion( self, ... )
		return sendChatText( self:GetNearPlayers( EmotionDistance:GetInt(), true, false, false ), self, CHAT_EMOTION, self:Nick(), self:Alive(), ... )
	end
...
local function ooc( self, text, isTeamChat )
...
do
	...
	---@param ignore_speaker boolean
	chat_Whisper = function( ply, message, team_chat, ignore_speaker )
		return sendChatText( ply:GetNearPlayers( MaxWhisperDistance:GetInt(), true, ignore_speaker, false ), ply, CHAT_WHISPER, message, ply:Nick(), ply:Alive(), team_chat and ply:Team() or false )
	end

	Jailbreak.Whisper = chat_Whisper
end

setChatCommand( "w", chat_Whisper, "#jb.chat.command.whisper" )
setChatCommand( "whisper", chat_Whisper, "#jb.chat.command.whisper" )
...
do
	...
	function GM:PlayerSay( ply, text, isTeamChat )
		...
		elseif ply:UsingSecurityRadio() then
			...
			chat_Whisper( ply, text, false, true )
			return ""
		end
